...
每日 **21:00–23:00（北京时间 / UTC+8）多时段自动重试** 由**云端（GitHub Actions）**自动执行：获取《新闻联播》文字稿 →
...
```
GitHub Actions 定时触发（北京 21:00/21:30/22:00/22:30/23:00 多班重试）
   │
   ▼ ① 采集（多源交叉核对）
...
├── .github/workflows/
│   └── daily_report.yml         # GitHub Actions 工作流（21:00-23:00 北京多班重试）
├── cloud/
